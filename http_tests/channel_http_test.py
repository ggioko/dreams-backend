...
def test_channel_leave():
    ...
    # Leaving user not in channel - Access Error
    test_2 = requests.post(config.url + 'channel/leave/v1', json={'token': rego_3['token'],\
    'channel_id': new_channel['channel_id']})
    assert test_2.status_code == AccessError().code

    # Success
    test_3 = requests.post(config.url + 'channel/leave/v1', json={'token': rego_2['token'],\
    'channel_id': new_channel['channel_id']})
    assert test_3.status_code == 200
