...
def test_channel_invite():
    ...
    r = requests.post(config.url + 'auth/register/v2', params = {'email':'madladadmin@gmail.com',\
    'password':'123abc!@#', 'name_first':'Hayden', 'name_last':'Everest'})
    rego_1 = r.json()
    r = requests.post(config.url + 'auth/register/v2', params = {'email':'peasantuser@gmail.com',\
    ...
    rego_3 = r.json()
    r = requests.post(config.url + 'channels/create/v2', params = {'token':rego_1['token'],\
    'name':'dankmemechannel', 'is_public':False})
    new_channel = r.json()

    # invalid channel id - Input error
    test_1 = requests.post(config.url + 'channel/invite/v2', params = {'token': rego_1['token'],\
    ...
    test_2 = requests.post(config.url + 'channel/invite/v2', params = {'token': rego_1['token'],\
    'channel_id': new_channel['channel_id'], 'u_id':rego_2['auth_user_id'] + 1})
    assert test_2.status_code == 400

    # invalid inviter - Access error
    test_3 = requests.post(config.url + 'channel/invite/v2', params = {'token': rego_2['token'],\
    ...
    r = requests.post(config.url + 'auth/logout/v1', params = {'token':rego_1['token']})
    test_1 = requests.post(config.url + 'channel/invite/v2', params = {'token': rego_1['token'],\
    'channel_id': new_channel['channel_id'], 'u_id':rego_2['auth_user_id']})
    assert test_1.status_code == 403
